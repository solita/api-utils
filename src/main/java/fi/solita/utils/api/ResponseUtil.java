...
public abstract class ResponseUtil {
    ...
    @Deprecated
    public static <T> Apply<T,Map<String, Object>> toPropertiesExcluding(Iterable<? extends MetaNamedMember<T, ?>> allFields, MetaNamedMember<T, ?> field1) {
        return toPropertiesExcluding(allFields, newList(field1));
    }
    
    @Deprecated
    public static <T> Apply<T,Map<String, Object>> toPropertiesExcluding(Iterable<? extends MetaNamedMember<T, ?>> allFields, MetaNamedMember<T, ?> field1, MetaNamedMember<T, ?> field2) {
        return toPropertiesExcluding(allFields, newList(field1, field2));
    }
    
    @Deprecated
    public static <T> Apply<T,Map<String, Object>> toPropertiesExcluding(Iterable<? extends MetaNamedMember<T, ?>> allFields, MetaNamedMember<T, ?> field1, MetaNamedMember<T, ?> field2, MetaNamedMember<T, ?>... exclude) {
        return toPropertiesExcluding(allFields, cons(field1, cons(field2, exclude)));
    }
    
    @Deprecated
    public static <T> Apply<T,Map<String, Object>> toPropertiesExcluding(Iterable<? extends MetaNamedMember<T, ?>> allFields, Iterable<? extends MetaNamedMember<T, ?>> exclude) {
        return toProperties(exclude(allFields, newList(exclude)));
    }

    @Deprecated
    public static <T> Apply<T,Map<String, Object>> toProperties(final Iterable<? extends MetaNamedMember<T, ?>> fields) {
        return new Apply<T,Map<String, Object>>() {
            @Override
            public Map<String, Object> apply(T t) {
                Map<String, Object> ret = newMapOfSize(size(fields));
                for (MetaNamedMember<T,?> f: fields) {
                    ret.put(f.getName(), f.apply(t));
                }
                return ret;
            }
        };
    ...
    @SuppressWarnings("unchecked")
    @Deprecated
    public static <T> Collection<MetaNamedMember<T, ?>> exclude(Iterable<? extends MetaNamedMember<T, ?>> allFields, Iterable<? extends MetaNamedMember<T, ?>> exclude) {
        Iterable<MetaNamedMember<T, Object>> ret = (Iterable<MetaNamedMember<T, Object>>) allFields;
        for (MetaNamedMember<T,Object> e: (Iterable<MetaNamedMember<T,Object>>)exclude) {
            ret = filter(not(equalTo(e)), ret);
        }
        return (Collection<MetaNamedMember<T, ?>>)(Object)newList(ret);
    }
    
    @SuppressWarnings("unchecked")
    @Deprecated 
    public static <T> T withProperties(Builder<T> builder, Iterable<? extends MetaNamedMember<? super T,?>> members, T obj) {
        for (MetaNamedMember<? super T,?> member: members) {
            Object val = member.apply(obj);
            if (val != null) {
                builder = builder.with((MetaNamedMember<T,Object>)member, val);
            }
        }
