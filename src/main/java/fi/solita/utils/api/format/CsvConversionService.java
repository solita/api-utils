...
public class CsvConversionService {
    ...
    private Pair<byte[],Map<String,String>> serialize(String filename, Iterable<String> tableHeader, Iterable<Iterable<Cells>> tableBody) {
        ...
        return Pair.of(mkString("\r\n", map(x -> joinCells(map(CsvConversionService::escape, x)), cons(header, body))).getBytes(Charset.forName("UTF-8")),
    ...
    private <K,V,O> Iterable<Iterable<Cells>> mapBody(final Map<K, ? extends Iterable<V>> obj, final Iterable<? extends MetaNamedMember<V, O>> members) {
        return map((BiFunction<K,V,Iterable<Cells>>)CsvConversionService_.<K,V,O>mapBodyRow().ap(this, members), flatMap(CsvConversionService_.<K,V>flatKeyToValues(), obj.entrySet()));
    }
    
    <K,V,O> Iterable<Cells> mapBodyRow(Iterable<? extends MetaNamedMember<V, O>> members, K key, V value) {
        return cons(module.serialize(key), map((BiFunction<Object,Class<?>,Cells>)CsvModule_.serialize1().ap(module), map(CsvConversionService_.<V,O>foo().ap(value), members)));
    }
