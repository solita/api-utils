...
public abstract class HtmlConversionService {
    ...
    public HtmlTitle title(final String title, final Option<Count> count) {
    ...
    public <T> byte[] serialize(HttpServletRequest request, HtmlTitle title, final Collection<T> obj, final Iterable<? extends MetaNamedMember<T, ?>> members) {
        return serialize(title, header(members), regularBody(obj, members), request);
    }
    ...
    public <K,V> byte[] serializeWithoutKey(HttpServletRequest request, HtmlTitle title, final Map<K,? extends Iterable<V>> obj, Iterable<? extends MetaNamedMember<V, ?>> members) {
        ...
        return serialize(title, header(headers), mapBody(obj, (Iterable<MetaNamedMember<V,Object>>)members), request);
    }
            ...
            html.html()
                ...
                .head()
                  ...
                  .style().write(
                          ...
                          + "h1             { font-weight: lighter; margin-bottom: 0.3em; position: relative; }"
                          ...
                          + mkString(" ", sequence(
                           ...
                           newList((Apply<String,String>)(Object)prepend("@media only screen and (max-width: 800px) {").andThen(append("}")), (Apply<String,String>)HtmlConversionService_.split.apply(Function._, "}").andThen(HtmlConversionService_.<String>toList()).andThen(Transformers.map(prepend(".nested ").andThen(HtmlConversionService_.replaceAll.apply(_, ",", ", .nested ")).andThen(append("}")))).andThen(HtmlConversionService_.join.ap(" ")) )))
                        , false)
                  ._style()
                ...
                .body(class_("fi"))
                  ...
                  .footer()
                      ...
                      .span(class_("en").class_("copyright"))
                          .write("© Finnish Transport Agency")
                      ._span()
                  ._footer()
                  .script(type("text/javascript"))
    ...
    private <T> Renderable header(final Iterable<? extends MetaNamedMember<T, ?>> members) {
